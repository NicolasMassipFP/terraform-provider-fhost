...
resource "smc_fw_ipv4_access_rule" "allow_ssh" {
  from_ref = smc_fw_policy.tf_aaa_policy.lk.fw_ipv4_access_rules
  name     = "aaa"
  sources { any = true }
...
action "smc_initial_contact" "tf_single_fw_contact" {
  config {
    initial_contact_href = smc_single_fw.tf_single_fw.nodes[0].firewall_node.lk.initial_contact
    enable_ssh           = true
    time_zone            = "CET"
    keyboard             = "us"
    root_password        = "root"
...
action "smc_command" "bind_license" {
  config {
    ...
    parameters = {
      component_href = smc_single_fw.tf_single_fw.nodes[0].firewall_node.lk.self
    }
  }
}
...
action "smc_command" "upload" {
  config {
    command_href = smc_single_fw.tf_single_fw.lk[var.refresh ? "refresh" : "upload"]
    query = {
...
action "smc_command" "sginfo" {
  config {
    command_href = smc_single_fw.tf_single_fw.nodes[0].firewall_node.lk.sginfo
    method       = "get"
    # query = {
...
action "smc_command" "change_state" {
  config {
    command_href = smc_single_fw.tf_single_fw.nodes[0].firewall_node.lk[var.new_state]
    method       = "put"
  }
}
...
action "smc_command" "add_route" {
  config {
    command_href = smc_single_fw.tf_single_fw.lk.add_route
    parameters = {
...
action "smc_command" "backup_now" {
  config {
    command_href = smc_backup_task.tf_backup_task.lk.start
  }
}

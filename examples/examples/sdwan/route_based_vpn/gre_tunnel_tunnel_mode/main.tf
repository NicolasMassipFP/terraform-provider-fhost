...
data "smc_sub_href" "single_fw1_internal_gateway_ref" {
  from_ref = smc_single_fw.tf_single_fw1.lk.internal_gateway
  match    = "tf_single_fw1*"
}

data "smc_sub_href" "single_fw1_tunnel_interface_6483_ref" {
  from_ref = smc_single_fw.tf_single_fw1.lk.tunnel_interface
  match    = "*6483"
}
...
data "smc_sub_href" "single_fw2_internal_gateway_ref" {
  from_ref = smc_single_fw.tf_single_fw2.lk.internal_gateway
  match    = "tf_single_fw2*"
}

data "smc_sub_href" "single_fw2_tunnel_interface_4563_ref" {
  from_ref = smc_single_fw.tf_single_fw2.lk.tunnel_interface
  match    = "*4563"
}
...
resource "smc_routing_node" "tf_single_fw" {
  id    = smc_single_fw.tf_single_fw1.lk.routing
...
resource "smc_internal_endpoint" "ep_192_168_100_14" {
  id = format("%s#%s", smc_single_fw.tf_single_fw1.lk.internal_gateway,
...
resource "smc_routing_node" "tf_single_fw2" {
  id    = smc_single_fw.tf_single_fw2.lk.routing
  level = "engine_cluster"
  routing_node {
    level = "interface"
    name  = "Interface 0"
...
resource "smc_internal_endpoint" "ep_10_0_50_14" {
  id = format("%s#%s", smc_single_fw.tf_single_fw2.lk.internal_gateway,
...
resource "smc_rbvpn_tunnel" "gre_single_fw1_single_fw2" {
  ...
  tunnel_mode         = "gre"
  tunnel_mode_vpn_ref = smc_vpn.tf_sample_vpn.id
}
...
resource "smc_gateway_node" "tf_sample_gateway_1_node" {
  from_ref   = smc_vpn.tf_sample_vpn.lk.central_gateway_node
  name       = "tf_sample_gateway_node_1"
  node_usage = "central"
  gateway    = data.smc_sub_href.single_fw1_internal_gateway_ref.id
}
...
resource "smc_gateway_node" "tf_sample_gateway_2_node" {
  ...
  node_usage = "central"
  gateway    = data.smc_sub_href.single_fw2_internal_gateway_ref.id
}

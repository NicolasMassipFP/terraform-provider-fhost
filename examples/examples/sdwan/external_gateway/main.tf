...
data "smc_sub_href" "internal_gateway" {
  ...
  from_ref = smc_single_fw.tf_single_fw[each.key].lk.internal_gateway
  match    = "tf_single_fw*"
}
...
resource "smc_gateway_node" "tf_sample_satellite_gateway2" {
  from_ref   = smc_vpn.tf_sample_vpn.lk.satellite_gateway_node
...
resource "smc_internal_endpoint" "ep_fw1" {
  id = format("%s#%s", smc_single_fw.tf_single_fw["fw1"].lk.internal_gateway,
...
resource "smc_internal_endpoint" "ep_fw2" {
  id = format("%s#%s", smc_single_fw.tf_single_fw["fw2"].lk.internal_gateway,
...
resource "smc_vpn_site" "site_fw1" {
  from_ref     = format("%s#%s", smc_single_fw.tf_single_fw["fw1"].lk.internal_gateway, "*/vpn_site")
  name         = "site_fw1"
...
resource "smc_vpn_site" "site_external" {
  from_ref     = smc_external_gateway.external_gateway.lk.vpn_site
  name         = "external_site"
  site_element = [smc_network.host2.id]
}

resource "smc_external_endpoint" "ep_external" {
  from_ref  = smc_external_gateway.external_gateway.lk.external_endpoint
  enabled   = true
  address   = "10.25.25.15"
  ipsec_vpn = true
}

...
- `activationDate` (Number) The date when the schedule is activated, represented as a timestamp in milliseconds since epoch.
...
- `dayPeriod` (String) The period of the schedule, which can be 'one_shot' for no repeat, 'daily' for every day, 'weekly' for every week, 'monthly' for every month, or 'yearly' for every year.

...
- `failover_time` (Number) The failover time in milliseconds for clustering heartbeat messages. This is the time after which a node is considered failed if no heartbeat is received.
...
- `master_type` (String) The type of the Master Engine, which defines its role in the cluster. Currently, only 'dcl2fw' is supported for Data Center Layer 2 type.
- `message_period` (Number) The period in milliseconds for clustering heartbeat messages.
...
- `virtualResources` (List of Blocks, see [here](../attributes/abstract_virtual_resource.md)) A list of Virtual Resources that associate Virtual NGFW Engines with interfaces on the Master NGFW Engine. You must create one Virtual Resource for each Virtual NGFW Engine that you plan to add.

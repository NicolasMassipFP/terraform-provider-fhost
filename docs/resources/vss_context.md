...
- `domain_server_address` (List of Blocks, see [here](../attributes/dns_element.md)) Defines one or more DNS IP Addresses. These are the IP addresses of the DNS server(s) that the IPS uses to resolve virus signature mirrors, domain names, and web filtering categorization services (which are defined as URLs). You can enter a single IP address manually or define an IP address using a network element.
...
- `vc_isc` (Single Block, see [here](../attributes/vss_context_isc_settings.md)) 

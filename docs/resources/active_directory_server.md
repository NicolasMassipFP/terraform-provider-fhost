...
- `domain_controller` (List of Blocks, see [here](../attributes/domain_controller.md)) The Domain Controllers associated with the Active Directory Server. Users are associated with IP addresses based on logs collected by the Active Directory Domain Controller. It is not recommended to use this feature for terminal servers or other computers that have many users logged on at the same time.

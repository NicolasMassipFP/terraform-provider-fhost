...
- `block_list_scope` (List of Blocks, see [here](../attributes/block_list_scope.md)) A block list scope, which define the entry that stops matching connections and block traffic between the matching IP addresses for the set duration.
- `connection_tracking_options` (Single Block, see [here](../attributes/connection_tracking_options.md)) 
